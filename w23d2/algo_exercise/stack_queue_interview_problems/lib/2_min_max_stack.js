...
push(val) {
    ...
    if (this._min.length === 0 || 
        newNode.value <= this._min[this._min.length - 1].value) {
            this._min.push(newNode); 
    }

    if (this._max.length === 0 ||
        newNode.value >= this._max[this._max.length - 1].value) {
            this._max.push(newNode); 
    } 
...
pop() {
    ...
    if (this.top.value === this._min[this._min.length - 1].value) {
        this._min.pop();
    }

    if (this.top.value === this._max[this._max.length - 1].value) {
        this._max.pop();
    }
...
min() {
    const minimum = this._min[this._min.length - 1];
    if (minimum) {
        return minimum;
    }else{
        return null;
...
max() {
    const maximum = this._max[this._max.length - 1]; 
