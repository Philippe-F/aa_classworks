...
function fileFinder(directory, targetFile) {
    if (directory[targetFile] === null) return true; 
    let found = false;
    for (let dir in directory) {
        if (dir[0] === '/') {
            found = fileFinder(directory[dir], targetFile);
            if (found) break;
        }
    }
    return found;
...
function pathFinder(directories, targetFile) {
    if (directories[targetFile] === null) return `/${targetFile}`; // '/example.jpeg'
    let found = null;
    for (let filename in directories) {
        if (directories[filename] === null) continue;
        found = pathFinder(directories[filename], targetFile);
        if (found !== null) {
